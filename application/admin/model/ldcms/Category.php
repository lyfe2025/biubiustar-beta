...
class Category extends Backend
{
    ...
    // 保存自定义的urlname
    protected static function updateCustomUrlname()
    {
        $config = get_addon_fullconfig('ldcms');
        $urlnames = Db::name('ldcms_category')->where('id', '>', 0)->column('urlname,mid', 'id');
        $customUrlNames = [];
        foreach ($urlnames as $key => $row) {
            $urlname = $row['urlname'];
            if (strpos($urlname, '/') !== false && strpos($urlname, '@') !== 0) {
                $customUrlNames[$urlname] = 'ldcms/lists/index';
                if ($row['mid'] != 1) {
                    $customUrlNames[$urlname.'/[:id]'] = 'ldcms/detail/index';
                }
            }
        }
        foreach ($config as &$conf) {
            if ($conf['name'] == 'custom_urlname') {
                $conf['value'] = $customUrlNames;
            }
        }
        unset($conf);
        // 更新配置文件
        set_addon_fullconfig('ldcms', $config);
