...
class Slider extends Base
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\ldcms\Ad();
    ...
    public function add()
    {
        ...
        return Form::instance()
            ...
            ->setFormItem('type', '类型', 'selectpage', 'required', ['placeholder' => '仅支持字母、数字、下划线','content_list' => 'ldcms/slider/selectpageType'])
            ->setFormItem('title', '标题', 'string', 'required')
            ->setFormItem('is_video', '使用视频', 'radio', 'required', [
                'content_list' => [ 0 => '否',1 => '是'] ,'value' => 0,
            ])
            ->setFormItem('image', '图片', 'image', 'required')
            ->setFormItem('video', '上传视频', 'file', 'required', ['visible' => 'is_video=1'])
            ->setFormItem('url', '链接', 'string', '', ['value' => '#'])
            ->setFormItem('target', '新窗口打开', 'string', '', ['value' => '_self'])
            ->setFormItem('description', '描述', 'text')
            ->setFormItem('content', '内容', 'text')
